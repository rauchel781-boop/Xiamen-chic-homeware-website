...
// WP stores a handful of HTML entities inside meta_title / meta_desc
// (e.g. "Cutlery &amp; Utensils"). stripHtml only removes tags, so without
// this the raw "&amp;" leaked straight into the <title> shown in the SERP.
...
// A few WP meta_titles are stored all-lowercase ("wooden watch box wholesale").
// Rendered as-is they look broken in the SERP, so title-case them — but ONLY
// when the string contains no uppercase at all, so deliberate casing (OEM,
// ODM, FSC, MOQ, brand names) is never clobbered.
function fixCase(t) {
  // Segment-wise, because titles like "wooden watch box wholesale | Custom
  // Logo Display Case" have an all-lowercase FIRST segment while the string
  // as a whole already contains uppercase.
  return t
    .split(/( \| | \u2014 | \u2013 | - )/)
...
function productSeoTitle(metaTitle, rawTitle) {
  let t = fixCase(decodeEntities(stripHtml(metaTitle || rawTitle || '')).replace(/\s+/g, ' ').trim());
  if (t.length <= 60) return t;

  // Longest separator-delimited prefix that still fits. The old code took the
  // FIRST separator it found, which threw away up to 35 characters of usable
  // title space — "Wooden Watch Box 12 Slot" instead of the full 60 chars
  // Google will actually render.
  let sepCut = '';
  for (const sep of [' | ', ' — ', ' – ', ' - ']) {
    const i = t.indexOf(sep);
    if (i >= 20 && i <= 60 && i > sepCut.length) sepCut = t.slice(0, i).trim();
  }

  // Word-boundary cut at 60, with any dangling separator character removed.
...
function ProductView({ p, locale }) {
  ...
  const productLd = {
    ...
    offers: {
      ...
      // offerCount is a REQUIRED property of AggregateOffer for Google's
      // Product rich result. Without it the offer block validates as schema.org
      // but is dropped by Google's parser — which is the most likely reason the
      // "Product snippet" appearance in Search Console fell off a cliff.
      // One aggregated wholesale band per product, so the honest value is 1.
      offerCount: 1,
