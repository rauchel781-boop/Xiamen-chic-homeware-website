...
// "Industry Brief" posts live in posts.json alongside regular blog posts but
// render under /briefs/<slug> with a different template. Use wpBlogPosts()
// in the /blog routes and wpBriefs() at /briefs to keep the two streams
// from cross-polluting (and to avoid duplicate canonical URLs).
function isBrief(p) {
  return p.type === 'brief'
    || (p.categories || []).some(c => c.slug === 'industry-brief');
}
export const wpBlogPosts = () => wpPosts().filter(p => !isBrief(p));
export const wpBriefs    = () => wpPosts().filter(isBrief);
...
export function wpBlogPostBySlug(slug)   { return wpBlogPosts().find(p => p.slug === slug); }
export function wpBriefBySlug(slug)      { return wpBriefs().find(p => p.slug === slug); }
...
// IMPORTANT: in wp-data/product_categories.json the `parent` field holds the
// parent's SLUG (e.g. "gift-boxes-retail-packaging"), not its numeric id.
// Code that assumed an id silently produced empty results — parent category
// pages showed only directly-assigned products and never listed their
// subcategories. These helpers accept either form so both work.
function parentMatches(cat, parentCat) {
  if (!cat.parent) return false;
  return String(cat.parent) === String(parentCat.slug)
      || String(cat.parent) === String(parentCat.id);
}

/** Direct children of a category. */
export function wpCategoryChildren(cat) {
  if (!cat) return [];
  return wpProductCategories().filter(c => c.slug !== 'uncategorized' && parentMatches(c, cat));
}

/** A category plus every descendant, breadth-first, de-duplicated. */
export function wpCategoryWithDescendants(cat) {
  if (!cat) return [];
  const out = [cat];
  const seen = new Set([cat.slug]);
  for (let i = 0; i < out.length; i++) {
    for (const child of wpCategoryChildren(out[i])) {
 ...
 *
 * The six top-level navigation categories are parents whose products live in
 * their children, so the direct-only query left them nearly empty:
 * desk-office-organizers showed 2 products instead of 24, and
 * gift-boxes-retail-packaging showed 3 instead of 40. These are the site's
 * highest-authority commercial pages, which is why the whole gift-box keyword
 * cluster sat at position 30-60.
 */
export function wpProductsByCategoryTree(cat) {
  const slugs = new Set(wpCategoryWithDescendants(cat).map(c => c.slug));
  return wpProducts().filter(p => p.categories?.some(c => slugs.has(c.slug)));
}

export function wpProductCategoryTree() {
  const cats = wpProductCategories().filter(c => c.slug !== 'uncategorized');
  // Index by BOTH slug and id — `parent` holds a slug in our data, but older
  // exports used ids, so accept either rather than silently flattening the
  // tree (parseInt('gift-boxes-retail-packaging') is NaN, which made every
  // category look like a root).
  const nodes = cats.map(c => ({ ...c, children: [] }));
  const bySlug = Object.fromEntries(nodes.map(c => [String(c.slug), c]));
  const byId   = Object.fromEntries(nodes.map(c => [String(c.id), c]));
  const roots = [];
  for (const c of nodes) {
    const key = c.parent == null ? '' : String(c.parent);
    const parent = key && (bySlug[key] || byId[key]);
...
export function wpCleanPageHtml(html) {
  ...
  h = h.replace(
    ...
    '<a href="/contact#form" class="wp-cta-btn">$1</a>'
  );
