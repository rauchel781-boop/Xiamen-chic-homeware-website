...
export const PRICING_BY_CATEGORY = {
  ...
  'pet-urns':                        { moq: 300, lowPrice: 3.50,  highPrice: 14.00 },
  'pet-memorial-keepsakes':          { moq: 300, lowPrice: 1.20,  highPrice: 12.00 },
  'pet-storage-feeding':             { moq: 300, lowPrice: 3.00,  highPrice: 10.00 },
  'pet-houses-habitats':             { moq: 300, lowPrice: 4.00,  highPrice: 14.00 },
  // ── Added 2026-09-08 with the homeware-0908 intake ────────────────
  // NOTE: these three bands are ESTIMATES pending factory confirmation.
  // Everything above came from the factory; these did not. Side tables and
  // fitted picnic hampers carry hardware and freight profiles the existing
  // tray / kitchen bands do not cover, so leaving them on those bands would
  // have published a materially wrong AggregateOffer.
  'wooden-side-tables':              { moq: 100, lowPrice: 12.00, highPrice: 32.00 },
  'picnic-baskets-hampers':          { moq: 300, lowPrice: 8.00,  highPrice: 22.00 },
  'wooden-display-racks':            { moq: 300, lowPrice: 6.00,  highPrice: 15.00 },
  // ── Added 2026-09-14 with the homeware-0914 intake ────────────────
  // Also ESTIMATES pending factory confirmation. Valet trays sit slightly
  // above the desk-organizer band because the lined and leather versions
  // carry a material cost the bare trays do not; pantry organizers mirror
  // the spice-rack band they are built alongside.
  'wooden-valet-trays':              { moq: 300, lowPrice: 2.50,  highPrice: 10.00 },
  'wooden-pantry-organizers':        { moq: 300, lowPrice: 2.00,  highPrice: 9.00  },
  'wooden-document-trays':           { moq: 300, lowPrice: 2.00,  highPrice: 8.00  },
  'wooden-cable-organizers':         { moq: 300, lowPrice: 1.00,  highPrice: 5.00  },
  'wooden-coffee-box':               { moq: 300, lowPrice: 2.00,  highPrice: 9.00  },
  'wooden-vinyl-record-holders':     { moq: 300, lowPrice: 5.00,  highPrice: 16.00 },
  'branded-wooden-promotional-gifts':{ moq: 300, lowPrice: 1.00,  highPrice: 8.00  },
  'airbnb-welcome-trays':            { moq: 300, lowPrice: 1.50,  highPrice: 8.00  },
  // Fallback bucket
...
// A category band is set for the TYPICAL item in that category. When a
// specific product sits well outside it — a solid-walnut cable box in a
// category priced for small desk accessories, a gift set whose bought-in
// glass costs more than most of our boxes — the band publishes a
// misleading "From $X" in the Product schema and pulls in enquiries at
// the wrong budget. These entries override the band for that slug only.
//
// STATUS: estimates agreed with the factory on 2026-09-15, pending the
// formal quotation. Update the numbers here; nothing else needs to change.
export const PRICING_BY_SLUG = {
  // 0915 intake — cable management and charging
  'acacia-cable-management-box-with-legs':      { moq: 300, lowPrice: 6.00,  highPrice: 18.00 },
  'walnut-cable-management-box':                { moq: 300, lowPrice: 6.00,  highPrice: 18.00 },
  'oval-wooden-cable-box-with-lid':             { moq: 300, lowPrice: 6.00,  highPrice: 18.00 },
  'bamboo-charging-station-with-pegboard':      { moq: 300, lowPrice: 6.00,  highPrice: 18.00 },
  'house-shaped-router-storage-box':            { moq: 300, lowPrice: 6.00,  highPrice: 18.00 },
  'wooden-cabinet-with-fluted-glass-doors':     { moq: 300, lowPrice: 12.00, highPrice: 30.00 },

  // 0915 intake — sets carrying bought-in components
  'walnut-whiskey-stones-gift-set-box':         { moq: 300, lowPrice: 6.00,  highPrice: 14.00 },
  'carved-lattice-wooden-gift-box':             { moq: 300, lowPrice: 2.50,  highPrice: 9.00  },
  'rattan-inlay-wooden-serving-tray-set':       { moq: 300, lowPrice: 5.00,  highPrice: 14.00 },
  'rustic-wood-tray-with-glass-base':           { moq: 300, lowPrice: 5.00,  highPrice: 14.00 },

  // 0915 intake — mixed wood-and-metal construction
  'two-tier-wood-and-metal-vanity-shelf':       { moq: 300, lowPrice: 7.00,  highPrice: 16.00 },
  'wood-and-metal-file-organizer-with-drawer':  { moq: 300, lowPrice: 7.00,  highPrice: 16.00 },

  // 0914 intake — presentation boxes that were inheriting the small
  // gift-box band and publishing an implausible $0.50 floor.
  'beech-slide-open-presentation-box':          { moq: 300, lowPrice: 1.50,  highPrice: 8.00  },
  'uv-printed-wooden-gift-keepsake-box':        { moq: 300, lowPrice: 1.50,  highPrice: 8.00  },
};

export function getPriceRangeForProduct(product) {
  // A per-product override always wins over the category band.
  if (product?.slug && PRICING_BY_SLUG[product.slug]) {
    return PRICING_BY_SLUG[product.slug];
  }
