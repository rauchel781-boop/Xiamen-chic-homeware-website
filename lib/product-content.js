...
const TEMPLATE_KEYS = [
  // ── Pet range ──────────────────────────────────────────────────────
  // Listed FIRST so pet products win over the generic giftBox/storageBox
  // templates, whose keywords ('keepsake box', 'storage box') would
  // otherwise swallow them.
  { key: 'petUrn',      catSlugContains: 'pet-urns',       faqCount: 3,
    keywords: ['pet urn', 'cremation urn', 'urn for ashes', 'pet ashes', 'memorial urn'] },
  { key: 'petMemorial', catSlugContains: 'pet-memorial',   faqCount: 2,
    keywords: ['pet memorial', 'pet keepsake', 'pet casket', 'pet burial', 'fur keepsake', 'paw print'] },
  { key: 'petStorage',  catSlugContains: 'pet-storage',    faqCount: 2,
    keywords: ['pet treat', 'dog treat', 'cat treat', 'pet food storage', 'treat canister', 'treat jar'] },
  { key: 'petHouse',    catSlugContains: 'pet-houses',     faqCount: 2,
    keywords: ['bat house', 'squirrel house', 'hamster house', 'small pet house', 'nesting box', 'bird house', 'animal house'] },
  ...
  { key: 'cuttingBoard', catSlugContains: 'cutting-board', faqCount: 1,
    ...
    // A rack that HOLDS boards is not a board. Without this guard, the
    // "Rotating Pot Lid Organizer - Wooden Cutting Board and Lid Rack"
    // inherited the cutting-board FAQ and answered questions about oiling
    // and end-grain care on a product that has neither.
    notKeywords: ['pot lid', 'board organizer', 'board organiser', 'board rack', 'board holder'] },
  ...
  { key: 'teaBox',       catSlugContains: 'tea-box',       faqCount: 1,
    // 'tea bag' alone was too broad: it pulled the tea-box FAQ (hinged lid,
    // glass viewing window) onto open sachet caddies that have neither.
    // Products genuinely in the tea-box category still match on catSlug.
    keywords: ['tea box', 'tea bag box'] },
...
function detectTemplate(title, productCategories) {
  ...
  for (const tpl of TEMPLATE_KEYS) {
    // A notKeywords hit vetoes the template outright, including a category
    // match, because the title is the more specific signal about what the
    // product actually IS.
    if (tpl.notKeywords && tpl.notKeywords.some(kw => t.includes(kw))) continue;
