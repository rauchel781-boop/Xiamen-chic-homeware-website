  ...
  return '<table style="width:100%;background:#fff;border:1px solid #e5e7eb;">' +
    ...
    items.map((it, idx) => {
      ...
      return '<tr>' +
        ...
        '<td class="text-center">' + (p && p.image ? '<img src="' + p.image + '" style="width:42px;height:42px;object-fit:contain;background:#f9fafb;border-radius:3px;">' : '<span class="muted">-</span>') + '</td>' +
...
async function addProductImage(wb, ws, cellRef, productImage, w, h) {
  ...
  try {
    const imgId = wb.addImage({ base64: productImage, extension: 'png' });
    const m = cellRef.match(/^([A-Z]+)(\d+)$/);
    if (!m) return;
    const col = m[1].charCodeAt(0) - 'A'.charCodeAt(0);
    const row = parseInt(m[2]) - 1;
    ws.addImage(imgId, { tl: { col: col + 0.1, row: row + 0.1 }, ext: { width: w, height: h } });
        ...
        <table style="width:100%;">
          ...
          ${list.map(p => {
            ...
            return `<tr style="cursor:pointer;${checked ? 'background:#eff6ff;' : ''}" ${rowClick}>
              ...
              <td>${p.image ? `<img src="${p.image}" style="width:40px;height:40px;object-fit:contain;background:#f9fafb;border-radius:3px;">` : '<div style="width:40px;height:40px;background:#f3f4f6;border-radius:3px;"></div>'}</td>
              <td class="code">${escapeHtml(p.code || '-')}</td>
              <td>${escapeHtml(p.nameZh || '-')}</td>
