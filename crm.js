...
function render() {
  const fn = ({
    ...
    samples: renderSamples, orders: renderOrders, purchases: renderPurchases, payments: renderPayments, shipments: renderShipments, followups: renderFollowups,
...
function addQtItem() {
  if (DB.products.length === 0) {
    window.__qtItems.push({ _tmpId: uid(), productId: '__custom', customName: '', specs: '', qty: 1, price: 0 });
    renderQtItems();
    return;
  }
...
function renderPayments() {
  document.getElementById('pageTitle').textContent = '收款 / 付款';
  document.getElementById('topbarActions').innerHTML = `
...
function renderPaymentForm() {
  ...
  return `
    <div class="form-grid">
      ...
      <div class="field"><label>金额 <span class="req">*</span></label>
        <input type="number" min="0" step="0.01" value="${escapeHtml(p.amount || '')}" oninput="_editingPayment.amount=this.value"></div>
      <div class="field"><label>币种</label>
