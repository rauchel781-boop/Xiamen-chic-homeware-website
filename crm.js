...
function renderRichText(s) {
  if (!s) return '';
  if (isHtml(s)) return sanitizeRichHtml(s);
  return nl2br(s);
}
...
function richTextEditor(name, raw, opts) {
  ...
  const initial = isHtml(raw) ? sanitizeRichHtml(raw) : escapeHtml(raw || '').replace(/\n/g, '<br>');
  ...
  if (b.type === 'image') {
    ...
    if (editing) {
      return `<div class="dossier-image-wrap">
        ${src ? `<img src="${src}">` : '<div class="dossier-image-empty">尚未上传图片</div>'}
      ...
      <div class="split-main">
        ...
        <div style="flex:1;overflow:auto;">
          ...
          <table>
            ...
            ${list.map(p => `
              <tr>
                <td>${p.image ? `<img src="${p.image}" class="product-thumb">` : '<div class="product-thumb"></div>'}</td>
  ...
  openModal(id ? '编辑产品' : '新建产品', `
    <form id="productForm" onsubmit="return saveProduct(event, '${id || ''}')">
      <div style="display:flex;gap:18px;margin-bottom:14px;">
        <div>
          ...
          <div id="imgWrap" tabindex="0" class="product-img-drop"
            ...
            ${p.image ? `<img src="${p.image}" class="product-thumb-large" onclick="document.getElementById('productImage').click()">` :
      ...
      <div class="form-grid">
        ...
        <div class="field"><label>HS 编码</label>
          <input name="hsCode" value="${escapeHtml(p.hsCode || '')}"></div>
        <div class="field"><label>MOQ</label>
          <input name="moq" value="${escapeHtml(p.moq || '')}"></div>
        <div class="field"><label>包装</label>
          <input name="packing" value="${escapeHtml(p.packing || '')}"></div>
        ...
        <div class="field full"><label>产品描述</label>
...
function processProductImageFile(file) {
  ...
  // 用更高质量的压缩（1200px，比原 handleImageFile 的 400px 清晰）
  compressImgFile(file, dataUrl => {
    ...
    if (wrap) {
      ...
      wrap.innerHTML = `<img src="${dataUrl}" class="product-thumb-large" onclick="document.getElementById('productImage').click()">`;
    }
    toast('图片已加载 (' + Math.round(dataUrl.length / 1024) + 'KB)', 'success');
  });
}
...
function viewProduct(id) {
  ...
  openModal(p.nameEn || p.nameZh || '产品详情', `
    <div style="display:flex;gap:18px;">
      <div>${p.image ? `<img src="${p.image}" class="product-thumb-large">` : '<div class="product-thumb-large"></div>'}</div>
      <div style="flex:1;">
        <dl class="detail-grid">
          ...
          <dt>HS编码</dt><dd>${escapeHtml(p.hsCode || '-')}</dd>
          <dt>MOQ</dt><dd>${escapeHtml(p.moq || '-')}</dd>
          <dt>包装</dt><dd>${escapeHtml(p.packing || '-')}</dd>
          ...
          <dt>描述</dt><dd>${nl2br(p.description || '-')}</dd>
        </dl>
      </div>
    </div>
  `, `<button class="btn" onclick="closeModal()">关闭</button>
...
let sampleFilter = '', sampleStatusFilter = '';

function renderSamples() {
  document.getElementById('pageTitle').textContent = '样品管理';
  document.getElementById('topbarActions').innerHTML = `<button class="btn btn-primary" onclick="editSample()">+ 新建样品</button>`;
  setTabs('');
  const kw = sampleFilter.toLowerCase();
  const list = DB.samples.filter(s => {
    const c = customerById(s.customerId);
    return (!kw || (s.sampleNo||'').toLowerCase().includes(kw) || (s.productName||'').toLowerCase().includes(kw) || (c && c.company.toLowerCase().includes(kw)))
  ...
  document.getElementById('content').innerHTML = `
    <div class="table-wrap">
      <div class="table-toolbar">
        <input class="search-box" placeholder="搜索 编号 / 产品 / 客户..." value="${escapeHtml(sampleFilter)}" oninput="sampleFilter=this.value;renderSamples()">
      ...
      ${list.length === 0 ? '<div class="empty">暂无样品记录</div>' : `
      <table>
        <thead><tr><th>编号</th><th>客户</th><th>产品</th><th>规格</th><th>寄出日期</th><th>快递</th><th>状态</th><th class="text-right">操作</th></tr></thead>
        <tbody>
        ${list.map(s => `<tr>
          <td class="code no-wrap">${escapeHtml(s.sampleNo || '')}</td>
          <td>${customerNameWithFlag(s.customerId)}</td>
          <td>${escapeHtml(s.productName || '')}</td>
          <td class="muted">${escapeHtml(s.specs || '')}</td>
          <td class="no-wrap">${fmtDate(s.sentDate)}</td>
          <td class="muted">${escapeHtml(s.trackingNo || '')}</td>
...
function editSample(id, customerId) {
  if (DB.customers.length === 0) { toast('请先添加客户', 'error'); return; }
  const s = id ? DB.samples.find(x => x.id === id) : { customerId: customerId || '', sentDate: todayStr(), status: '筹备中' };
  openModal(id ? '编辑样品' : '新建样品', `
    <form id="sampleForm" onsubmit="return saveSample(event, '${id || ''}')">
      <div class="form-grid">
        <div class="field"><label>样品编号</label>
          <input name="sampleNo" value="${escapeHtml(s.sampleNo || '')}" placeholder="留空自动生成"></div>
        <div class="field"><label>客户 <span class="req">*</span></label>
          <select name="customerId" required><option value="">请选择</option>${customerOptions(s.customerId)}</select></div>
        <div class="field full"><label>产品名称 <span class="req">*</span></label>
          <input name="productName" required value="${escapeHtml(s.productName || '')}" list="productNameList">
          <datalist id="productNameList">${DB.products.map(p => '<option value="' + escapeHtml(p.nameEn || p.nameZh) + '">').join('')}</datalist></div>
        <div class="field"><label>规格</label>
          <input name="specs" value="${escapeHtml(s.specs || '')}"></div>
        <div class="field"><label>样品成本</label>
          <input name="cost" value="${escapeHtml(s.cost || '')}"></div>
        <div class="field"><label>寄出日期</label>
          <input name="sentDate" type="date" value="${fmtDate(s.sentDate)}"></div>
        <div class="field"><label>状态</label>
          <select name="status">${SAMPLE_STATUSES.map(st => `<option ${s.status===st.name?'selected':''}>${st.name}</option>`).join('')}</select></div>
        <div class="field full"><label>快递公司 / 单号</label>
          <input name="trackingNo" value="${escapeHtml(s.trackingNo || '')}"></div>
        <div class="field full"><label>客户反馈 / 备注</label>
          <textarea name="feedback">${escapeHtml(s.feedback || '')}</textarea></div>
      </div>
    </form>
  `, `<button class="btn" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="document.getElementById('sampleForm').requestSubmit()">保存</button>`);
  if (s.customerId) { const sel = document.querySelector('#sampleForm [name=customerId]'); if (sel) sel.value = s.customerId; }
}

function saveSample(e, id) {
  e.preventDefault();
  const data = Object.fromEntries(new FormData(e.target).entries());
  if (id) Object.assign(DB.samples.find(x => x.id === id), data);
  else {
    if (!data.sampleNo) data.sampleNo = nextCode('SP');
    DB.samples.push({ id: uid(), createdAt: new Date().toISOString(), ...data });
  }
  saveDB(); closeModal(); renderSamples(); toast('已保存', 'success'); return false;
}
...
let orderFilter = '', orderPaymentFilter = '';
...
function renderOrders() {
  ...
  const list = DB.orders.filter(o => {
    const c = customerById(o.customerId);
    return (!kw || (o.orderNo||'').toLowerCase().includes(kw) || (o.items||'').toLowerCase().includes(kw) || (c && c.company.toLowerCase().includes(kw)))
        && (!orderPaymentFilter || o.paymentStatus === orderPaymentFilter);
  }).sort((a,b) => (b.orderDate||'').localeCompare(a.orderDate||''));

  const total = list.reduce((s, o) => s + (Number(o.amount) || 0), 0);
  const unpaid = list.filter(o => o.paymentStatus !== '已结清').reduce((s, o) => s + (Number(o.amount) || 0), 0);
  ...
  document.getElementById('content').innerHTML = `
    <div class="table-wrap">
      <div class="table-toolbar">
        ...
        <span class="muted">共 ${list.length} 条 / 合计 ${total.toLocaleString()} / 未结清 ${unpaid.toLocaleString()}</span>
      </div>
      ${list.length === 0 ? '<div class="empty">暂无订单</div>' : `
      <table>
        <thead><tr><th>订单号</th><th>客户</th><th>产品</th><th class="text-right">金额</th><th>付款</th><th>生产</th><th>交期</th><th class="text-right">操作</th></tr></thead>
        <tbody>
        ${list.map(o => `<tr>
          <td class="code"><strong>${escapeHtml(o.orderNo || '')}</strong>
            <div class="muted" style="font-size:10px;">${fmtDate(o.orderDate)}</div></td>
          <td>${customerNameWithFlag(o.customerId)}</td>
          <td class="muted">${escapeHtml(truncate(o.items, 35))}</td>
          <td class="text-right no-wrap"><strong>${escapeHtml(o.currency || '')} ${Number(o.amount || 0).toLocaleString()}</strong></td>
          <td><span class="tag ${getStatus(PAYMENT_STATUSES, o.paymentStatus).tag}">${escapeHtml(o.paymentStatus || '-')}</span></td>
...
function editOrder(id, customerId) {
  ...
  const o = id ? DB.orders.find(x => x.id === id) : {
    customerId: customerId || '', orderDate: todayStr(), currency: 'USD',
    paymentStatus: '未付款', productionStatus: '未开始'
  };
  openModal(id ? '编辑订单' : '新建订单', `
    <form id="orderForm" onsubmit="return saveOrder(event, '${id || ''}')">
      <div class="form-grid">
        <div class="field"><label>订单号</label>
          <input name="orderNo" value="${escapeHtml(o.orderNo || '')}" placeholder="留空自动生成"></div>
        <div class="field"><label>客户 <span class="req">*</span></label>
          <select name="customerId" required><option value="">请选择</option>${customerOptions(o.customerId)}</select></div>
        <div class="field"><label>下单日期</label><input name="orderDate" type="date" value="${fmtDate(o.orderDate)}"></div>
        <div class="field"><label>交货日期</label><input name="deliveryDate" type="date" value="${fmtDate(o.deliveryDate)}"></div>
        <div class="field"><label>金额</label><input name="amount" type="number" step="0.01" value="${escapeHtml(o.amount || '')}"></div>
        <div class="field"><label>币种</label>
          <select name="currency">${CURRENCIES.map(c => `<option ${o.currency===c?'selected':''}>${c}</option>`).join('')}</select></div>
        <div class="field"><label>付款状态</label>
          <select name="paymentStatus">${PAYMENT_STATUSES.map(s => `<option ${o.paymentStatus===s.name?'selected':''}>${s.name}</option>`).join('')}</select></div>
        <div class="field"><label>生产状态</label>
          <select name="productionStatus">${PRODUCTION_STATUSES.map(s => `<option ${o.productionStatus===s.name?'selected':''}>${s.name}</option>`).join('')}</select></div>
        <div class="field full"><label>产品明细</label>
          <textarea name="items" placeholder="产品A x 100pcs">${escapeHtml(o.items || '')}</textarea></div>
        <div class="field full"><label>备注</label>
          <textarea name="notes">${escapeHtml(o.notes || '')}</textarea></div>
      </div>
    </form>
  `, `<button class="btn" onclick="closeModal()">取消</button>
      <button class="btn btn-primary" onclick="document.getElementById('orderForm').requestSubmit()">保存</button>`, 'lg');
  if (o.customerId) { const sel = document.querySelector('#orderForm [name=customerId]'); if (sel) sel.value = o.customerId; }
}

function saveOrder(e, id) {
  e.preventDefault();
  const data = Object.fromEntries(new FormData(e.target).entries());
  if (id) Object.assign(DB.orders.find(x => x.id === id), data);
  else {
    if (!data.orderNo) data.orderNo = nextCode('SO');
    DB.orders.push({ id: uid(), createdAt: new Date().toISOString(), ...data });
  }
  saveDB(); closeModal(); renderOrders(); toast('已保存', 'success'); return false;
}
...
function shipProductCardHtml(item) {
  ...
  return '<div class="ship-product-card">' +
    (p.image ? '<img src="' + p.image + '">' : '<div class="no-img">无图</div>') +
    '<div class="info">' +
...
function openProductPicker(itemId) {
  ...
  setTimeout(() => { try { document.getElementById('productPickerSearch').focus(); } catch(e){} }, 80);
}

function closeProductPicker() {
  document.getElementById('productPickerMask').classList.remove('show');
  _productPickerItemId = null;
}

function renderProductPickerBody() {
  const kw = (document.getElementById('productPickerSearch').value || '').toLowerCase();
  const list = DB.products.filter(p =>
    !kw || (p.code||'').toLowerCase().includes(kw)
        || (p.nameEn||'').toLowerCase().includes(kw)
        || (p.nameZh||'').toLowerCase().includes(kw)
  );
  const body = document.getElementById('productPickerBody');
  if (list.length === 0) {
    body.innerHTML = '<div class="product-picker-empty">无匹配产品</div>';
    return;
  }
  body.innerHTML = list.map(p => {
    const packing = hasPackingInfo(p)
      ? p.qtyPerCarton + '/CTN · ' + calcCartonCBM(p).toFixed(4) + ' CBM'
      : '⚠ 未录入装箱';
    return '<div class="product-picker-card" onclick="selectProductForItem(\'' + p.id + '\')">' +
      (p.image ? '<img src="' + p.image + '">' : '<div class="no-img">无图</div>') +
      '<div class="code">' + escapeHtml(p.code || '-') + '</div>' +
      '<div class="name">' + escapeHtml(p.nameEn || p.nameZh || '-') + '</div>' +
      '<div class="packing ' + (hasPackingInfo(p) ? '' : 'packing-empty') + '">' + packing + '</div>' +
    '</div>';
  }).join('');
}

function selectProductForItem(productId) {
  if (!_productPickerItemId) return;
  const itemId = _productPickerItemId;
  const it = _editingShipment.items.find(x => x.id === itemId);
  if (!it) { closeProductPicker(); return; }
  it.productId = productId;
  // 重新渲染整行（因为按钮文字"选择"→"更换"也要变）
...
function addShipmentItem() {
  if (!_editingShipment) return;
  if (DB.products.length === 0) { toast('请先添加产品', 'error'); return; }
  const it = { id: uid(), productId: '', qty: '', tailMode: 'whole' };
  _editingShipment.items.push(it);
  const wrap = document.getElementById('shipItems');
  if (_editingShipment.items.length === 1) wrap.innerHTML = '';
  wrap.insertAdjacentHTML('beforeend', shipmentItemHtml(it));
  refreshShipmentTotal();
}
        ...
        <tbody>
          ${(s.items || []).map(it => {
            ...
            const imgCell = p && p.image
              ? '<td><img src="' + p.image + '" class="ship-detail-thumb"></td>'
...
function exportExcel() {
  const sheets = {
    ...
    '产品': DB.products.map(p => ({
      ...
      价格: p.price, 币种: p.currency, 规格: p.specs, HS编码: p.hsCode,
      MOQ: p.moq, 包装: p.packing, 描述: p.description
    })),
    '样品': DB.samples.map(s => ({
