...
function renderNav() {
  ...
  document.getElementById('nav').innerHTML = NAV_MENU.map(m => `
    <div class="nav-item ${m.id === currentPage ? 'active' : ''}" data-page="${m.id}">
      ...
      ${m.id === 'tasks' ? (function(){ var u=(DB.tasks||[]).filter(t=>!t.done && t.date <= todayStr()).length; return u ? '<span class="nav-badge">'+u+'</span>' : ''; })() : ''}
    </div>
  `).join('');
}
...
function renderDashboard() {
  ...
  const monthIncome = monthPayments.filter(p => p.type === 'income').reduce((s, p) => s + (Number(p.amount) || 0), 0);
  const monthExpense = monthPayments.filter(p => p.type === 'expense').reduce((s, p) => s + (Number(p.amount) || 0), 0);
  ...
  document.getElementById('content').innerHTML = `
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value green">${Math.round(monthIncome).toLocaleString()}</div>
        <div class="stat-sub">${monthPayments.filter(p=>p.type==='income').length} 笔（混合币种）</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">本月付款</div>
        <div class="stat-value red">${Math.round(monthExpense).toLocaleString()}</div>
        <div class="stat-sub">${monthPayments.filter(p=>p.type==='expense').length} 笔（混合币种）</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">本月净额</div>
        <div class="stat-value ${monthIncome - monthExpense >= 0 ? 'blue' : 'red'}">${Math.round(monthIncome - monthExpense).toLocaleString()}</div>
...
function renderCustomers() {
  ...
  const tabs = [
    ...
    { id: 'following',   name: '正在跟进', filter: c => c.status === '正在跟进' },
    { id: 'continuing',  name: '持续跟进', filter: c => c.status === '持续跟进' },
    { id: 'key',         name: '重点跟进', filter: c => c.status === '重点跟进' },
    { id: 'sample',      name: '已打样',   filter: c => c.status === '已打样' },
    { id: 'deal',        name: '已合作',   filter: c => c.status === '已合作' },
  ];
    ...
    <div class="table-wrap">
      ...
      <table>
        <thead><tr>
          ...
          <th>来源</th><th>区域</th><th>联系人</th><th>最后跟进</th>
...
function autoCreateTask(source, customerId, content) {
...
function saveQuotation(e, id) {
  ...
  if (id) Object.assign(DB.quotations.find(x => x.id === id), data);
    ...
    autoCreateTask('quotation', data.customerId, '报价 ' + data.code + (data.currency && data.totalAmount ? ' · ' + data.currency + ' ' + Number(data.totalAmount).toLocaleString() : ''));
  }
...
function saveSampleForm(id) {
  ...
  if (isNew) {
    ...
    autoCreateTask('sample', s.customerId, '打样 ' + s.code + ' · ' + cnt + ' 个产品');
  }
...
function saveOrderForm(id) {
  ...
  if (isNew) {
    autoCreateTask('order', o.customerId, '下单 ' + o.orderNo + (o.currency && o.amount ? ' · ' + o.currency + ' ' + Number(o.amount).toLocaleString() : ''));
  }
  try { saveDB(); } catch(err) { toast('保存失败：' + err.message, 'error'); return; }
  _editingOrder = null;
  closeModal();
...
function viewOrderReadonly(id) {
  ...
  const payments = (DB.payments || []).filter(p =>
    p.relatedType === 'order' && p.relatedNo === o.orderNo
  ).sort((a, b) => (a.date || '').localeCompare(b.date || ''));
  const paid = payments.reduce((s, p) => s + (Number(p.netAmount || p.amount) || 0), 0);
  ...
  openModal('订单详情 ' + (o.orderNo || ''), `
    ...
    ${payments.length > 0 ? `
    <div class="detail-section">
      <div class="detail-section-title">收款记录（${payments.length} 笔，共收 ${cur} ${paid.toFixed(2)}）</div>
      <table>
        <thead><tr><th>日期</th><th>编号</th><th>方式</th>
          <th class="text-right">金额</th><th class="text-right">手续费</th><th class="text-right">实收</th></tr></thead>
        <tbody>${payments.map(p => `<tr>
          ...
          <td class="text-right bold" style="color:#16a34a;">${p.currency} ${Number(p.netAmount || p.amount).toLocaleString()}</td>
        </tr>`).join('')}</tbody>
      </table>
    </div>` : ''}
...
function viewSampleReadonly(id) {
  ...
  const payments = (DB.payments || []).filter(p =>
    p.relatedType === 'sample' && p.relatedNo === s.code
  ).sort((a, b) => (a.date || '').localeCompare(b.date || ''));
...
let summaryUsdRate = 7.2;          // USD→CNY 汇率（用户可改）
...
function renderFinanceSummary() {
  ...
  all.forEach(p => {
    ...
    if (!buckets[k]) buckets[k] = { incomeUSD: 0, incomeCNY: 0, incomeOther: 0, expenseUSD: 0, expenseCNY: 0, expenseOther: 0 };
    const net = Number(p.netAmount || p.amount) || 0;
    const cur = p.currency || 'USD';
    const sign = p.type === 'income' ? 'income' : 'expense';
    if (cur === 'USD') buckets[k][sign + 'USD'] += net;
    else if (cur === 'CNY') buckets[k][sign + 'CNY'] += net;
    else buckets[k][sign + 'Other'] += net;
  });

  const keys = Object.keys(buckets).sort().reverse();

  // 用汇率折算计算利润（USD * rate + CNY）
  const rate = Number(summaryUsdRate) || 7.2;
  let grandIncome = 0, grandExpense = 0;
  keys.forEach(k => {
    const b = buckets[k];
    b.incomeCNYTotal = b.incomeUSD * rate + b.incomeCNY;
    b.expenseCNYTotal = b.expenseUSD * rate + b.expenseCNY;
  ...
  document.getElementById('content').innerHTML = `
    <div style="margin-bottom:14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      ...
      <span style="margin-left:14px;font-size:13px;color:#4b5563;">USD→CNY 汇率：</span>
      <input type="number" step="0.01" min="0" value="${rate}" oninput="summaryUsdRate=Number(this.value)||7.2;renderPayments()" style="width:80px;padding:4px 8px;border:1px solid #d1d5db;border-radius:3px;">
    ...
    <div class="table-wrap">
      ...
      <table>
        <thead><tr>
          ...
          <th class="text-right">收入 USD</th>
          <th class="text-right">收入 CNY</th>
          <th class="text-right" style="background:#dcfce7;">收入合计(¥)</th>
          <th class="text-right">支出 USD</th>
          <th class="text-right">支出 CNY</th>
        ...
        ${keys.map(k => {
          ...
          return `<tr>
            ...
            <td class="text-right no-wrap">${b.incomeUSD > 0 ? '$' + b.incomeUSD.toLocaleString() : '<span class="muted">-</span>'}</td>
            <td class="text-right no-wrap">${b.incomeCNY > 0 ? '¥' + b.incomeCNY.toLocaleString() : '<span class="muted">-</span>'}</td>
            ...
            <td class="text-right no-wrap">${b.expenseUSD > 0 ? '$' + b.expenseUSD.toLocaleString() : '<span class="muted">-</span>'}</td>
            <td class="text-right no-wrap">${b.expenseCNY > 0 ? '¥' + b.expenseCNY.toLocaleString() : '<span class="muted">-</span>'}</td>
    ...
    <div class="table-wrap">
      ...
      <table>
        <thead><tr>
          ...
          <th>关联</th><th>方式</th><th>凭证</th><th>备注</th>
          <th class="text-right">操作</th>
        </tr></thead>
        <tbody>
        ${list.map(p => `<tr>
          ...
          <td class="muted no-wrap">${p.relatedNo ? escapeHtml(p.relatedNo) : '-'}</td>
...
function renderPaymentForm() {
  ...
  return `
    <div class="form-grid">
      ...
      <div class="field full"><label>对方 (客户名 / 工厂名 / 其他)</label>
        <input value="${escapeHtml(p.counterparty || '')}" oninput="_editingPayment.counterparty=this.value" placeholder="如 Beauty Page Inc / 山东工厂"></div>
      <div class="field"><label>关联类型</label>
        <select onchange="_editingPayment.relatedType=this.value">
      ...
      <div class="field"><label>关联单号</label>
        <input value="${escapeHtml(p.relatedNo || '')}" oninput="_editingPayment.relatedNo=this.value" placeholder="如 SO202605001"></div>
...
function renderTasks() {
  ...
  const undoneCount = all.filter(t => !t.done).length;

  const kw = taskFilter.toLowerCase();
  let list = all.filter(t => {
    if (taskViewMode === 'undone' && t.done) return false;
    if (taskViewMode === 'done' && !t.done) return false;
    if (!kw) return true;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:10px;margin-bottom:12px;align-items:center;">
      <div class="task-view-tabs">
        <button class="task-view-tab ${taskViewMode==='undone' ? 'active' : ''}" onclick="taskViewMode='undone';renderTasks()">未完成 <span class="tag-count">${all.filter(t=>!t.done).length}</span></button>
...
function renderTaskRow(t) {
  ...
  const custHtml = c
    ? `<span class="task-cust-link" onclick="viewCustomerDetail('${c.id}')" title="查看客户">${escapeHtml(cName)}</span>`
  ...
  return `<div class="task-row ${t.done ? 'done' : ''}">
    ...
    <div class="task-actions">
      ${c && !t.done ? `<button onclick="convertTaskToFollowup('${t.id}')" title="转为客户跟进记录">→跟进</button>` : ''}
...
function quickAddTask() {
  ...
  lines.forEach(line => {
    DB.tasks.push({
      id: uid(), date, customerId, customerName,
...
async function saveTaskRich(e, id) {
  ...
  if (id) {
    const t = DB.tasks.find(x => x.id === id);
    if (!t) { toast('事项不存在', 'error'); return false; }
    Object.assign(t, { date: data.date, customerId: data.customerId, content: data.content });
  } else {
    DB.tasks.push({
      id: uid(), date: data.date, customerId: data.customerId, customerName: '',
      content: data.content, done: false, createdAt: new Date().toISOString()
    });
  }
...
function renderTodayTasksPanel() {
  ...
  const overdue = all.filter(t => !t.done && t.date && t.date < today);
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="panel" style="margin-bottom:12px;">
      ...
      <div class="panel-body">
        <div class="detail-grid">
          ...
          <dt>数据大小</dt><dd>${sizeMB} MB (${sizeKB} KB) <span class="muted" style="font-size:10px;">文字数据，已不含图片</span></dd>
          <dt>图片存储</dt><dd id="imgStorageInfo">加载中...</dd>
        </div>
      </div>
    </div>
    ...
    <div class="panel" style="margin-bottom:12px;">
      ...
      <div class="panel-body">
        ...
        <button class="btn btn-primary" onclick="exportData()">↓ 导出 JSON 备份 (仅数据)</button>
        <button class="btn btn-success" onclick="exportFullZip()" style="margin-left:6px;">↓ 导出完整 ZIP (数据+图片)</button>
...
async function exportFullZip() {
  ...
  try {
    ...
    for (const e of entries) {
      ...
      zip.file('images/' + e.id + '.' + ext, e.blob);
    }
    ...
    a.click();
    URL.revokeObjectURL(a.href);
    toast('已导出完整备份 (' + Math.round(blob.size / 1024 / 1024 * 10) / 10 + 'MB · 含 ' + entries.length + ' 张图)', 'success');
  } catch (err) {
    console.error(err);
